  ...
  : _use_high_res_timer(enable_high_res_timer)
{
  if (_use_high_res_timer) {
    ...
    if (QueryPerformanceCounter((LARGE_INTEGER*)&ctr) != 0) {
      ...
      std::cerr << "**************************************" << std::endl;
      std::cerr
        << "Error: Timer::Timer() unable to use QueryPerformanceFrequency, "
        << std::endl;
      std::cerr << "timing code will be wrong, Windows error code: "
...
Timer::tic() const
{
  if (_use_high_res_timer) {
    ...
    if (QueryPerformanceCounter((LARGE_INTEGER*)&qpc) != 0) {
      ...
      std::cerr
        << "Error: Timer::Timer() unable to use QueryPerformanceCounter, "
        << std::endl;
      std::cerr << "timing code will be wrong, Windows error code: "
                << GetLastError() << std::endl;
      return 0;
...
double
Timer::deltas(Timer_t t1, Timer_t t2) const
{
  if (_use_high_res_timer) {
    Timer_t frequency;
