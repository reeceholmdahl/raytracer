...
//
// I've also supplied a working windows timer using both the high resolution
// (nano-second)
// clocks and a lower resolution (millisecond) clock.  Supplying a "true" value
// to the constructor
...
public:
  ...
  inline Timer_t deltam(Timer_t t1, Timer_t t2) const
  {
    return Timer_t(deltas(t1, t2) * 1e3);
  }
  inline Timer_t deltau(Timer_t t1, Timer_t t2) const
  {
    return Timer_t(deltas(t1, t2) * 1e6);
  }
  inline Timer_t deltan(Timer_t t1, Timer_t t2) const
  {
    return Timer_t(deltas(t1, t2) * 1e9);
  }
