...
// Windows includes a macro called max that interfers with the numeric limits
// max functions.  This removes it.
#undef max
#endif
...
public:
  ...
  double taus()
  {
    ...
    double retVal = static_cast<double>(
      (tVal / static_cast<double>(std::numeric_limits<unsigned int>::max())));
    return retVal;
  }
...
private:
  ...
  double randVal() { return uniform_dist(m_prng); }
  ...
  unsigned long tausStep(unsigned long& z, int S1, int S2, int S3,
                         unsigned long M)
  {
