                     ...
                     const std::string& title)
{
  ...
  // Configure window
  glViewport(0, 0, windowWidth, windowHeight); // specifies the part of the
                                               // window to which OpenGL will
                                               // draw (in pixels), convert from
...
void
renderer::generateTexture(const Framebuffer& fb)
{
  glBindTexture(GL_TEXTURE_2D, tex);
...
renderer::drawFrame()
{
  ...
  glEnableClientState(GL_VERTEX_ARRAY_EXT); // tell OpenGL that you're using a
                                            ...
                                            // attribute
  glEnableClientState(GL_TEXTURE_COORD_ARRAY); // tell OpenGL that you're using
                                               // a vertex array for
...
void
renderer::changeCamera(const int camera)
{
  _camera = camera;
}
