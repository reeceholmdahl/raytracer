...
glm::mat4
Scene::parseTransformData(json& transformData)
{
  glm::mat4 glM(1.0f);
...
Shape*
Scene::extractAndCreateShapeFromJSONData(json& shapeData)
{
  ...
  if (type == "sphere") {
    ...
    shapePtr = new Sphere(center, radius);
...
#define MESHES_READY 0
    ...
    shapePtr = new OBJMesh(meshFile_fullPath, defaultShader, m_useBVH);
  }
#endif
...
#define INSTANCING_READY 0
    ...
    if (roIter != m_instanceMap.end()) {
      ...
      Matrix4x4 m(glM[0][0], glM[0][1], glM[0][2], glM[0][3], glM[1][0],
                  glM[1][1], glM[1][2], glM[1][3], glM[2][0], glM[2][1],
                  glM[2][2], glM[2][3], glM[3][0], glM[3][1], glM[3][2],
                  glM[3][3]);
      m = m.transpose();
      Matrix4x4 minv = m.inverse();

      shapePtr = new InstancedObject(robj, m, minv);

      if (!shapeData["shader"]["_ref"].empty()) {
        shapePtr->provideShader(locateShader(shapeData["shader"]["_ref"]));
      }
    }
  }
#endif
...
void
Scene::parseJsonFile(const fs::path& filename)
{
  Scene::parseJsonFile(filename.string());
}

void
Scene::parseJsonFile(const std::string& filename)
{
  ...
  if (!j["scene"]["_bgColor"].empty()) {
    bgColor = j["scene"]["_bgColor"];
  } else {
    bgColor = renderer::constants::DEFAULT_BG_COLOR;
  }
  ...
  if (jsonCameraStructure.is_array()) {
    std::cout << "Number of cameras: " << jsonCameraStructure.size()
              << std::endl;
    ...
    for (auto i = 0; i < jsonCameraStructure.size(); i++) {
      ...
      if (camType == "perspective") {
        camPtr = new PerspectiveCamera(position, viewDir, focalLength,
                                       imagePlaneWidth, aspectRatio);
      } else if (camType == "orthographic") {
        camPtr = new OrthographicCamera(position, viewDir, aspectRatio,
                                        imagePlaneWidth);
      }

      if (camPtr) {
        camPtr->setImagePixels(pixelsX, pixelsY);
        add(camPtr);
      }
    }
  }
  ...
  //
  // //////////////////////////////
  std::cout << "Number of shaders: " << j["scene"]["shader"].size()
            << std::endl;
...
#define ADVSHADERS_READY 0
      ...
      // boost::optional<std::string> pTexName =
      // v.second.get_optional<std::string>("diffuse.<xmlattr>.tex");
      // if (pTexName) {
      ...
      // pTexName =
      // v.second.get_optional<std::string>("specular.<xmlattr>.tex");
      // if (pTexName) {
      ...
      shaderPtr = new sivelab::BlinnPhongMirrored(kd, ks, phongExp, mirrorCoef,
                                                  roughnessCoef);
    }
#endif
  ...
  //
  // //////////////////////////////////////
  std::cout << "Number of Instance Objects: " << j["scene"]["instance"].size()
            << std::endl;
  ...
  for (auto i = 0; i < j["scene"]["shape"].size(); i++) {
    ...
    if (!shapeInfo["shader"]["_ref"].empty()) {
      ...
      shapePtr->shaderPtr = getShader(shaderName);
    }
...
#define ADVLIGHTS_READY 0
      ...
      AreaLight* aLight =
        new AreaLight(position, radiantEnergy, normal, width, length);
      ...
      sivelab::Vector3D v0 = position - aLight->binormal() * width / 2.0 -
                             aLight->tangent() * length / 2.0,
                        v1 = position - aLight->binormal() * width / 2.0 +
                             aLight->tangent() * length / 2.0,
                        v2 = position + aLight->binormal() * width / 2.0 +
                             aLight->tangent() * length / 2.0;
      ...
      v0 = position - aLight->binormal() * width / 2.0 -
           aLight->tangent() * length / 2.0;
      v1 = position + aLight->binormal() * width / 2.0 +
           aLight->tangent() * length / 2.0;
      v2 = position + aLight->binormal() * width / 2.0 -
