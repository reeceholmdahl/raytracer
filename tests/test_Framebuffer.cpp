...
main(int argc, char* argv[])
{
  ...
  for (auto i = 0; i < fb5.width(); ++i) {
    for (auto j = 0; j < fb5.height(); ++j) {
      ...
      fb5.setPixelColor(i, j, Vec3f(0.4 * (1.0 - light),
                                    0.4 * (1.0 - light) * sin(0.06 * j + 1.2) +
                                      0.5 * (1.0 - light),
                                    1.0 - light));
    }
  }
  ...
  for (auto i = 0; i < fb6.width(); ++i) {
    for (auto j = 0; j < fb6.height(); ++j) {
      ...
      double distance = sqrt(dx * dx + dy * dy);
      double light = std::min(1.0, distance / maxDistance);
      fb6.setPixelColor(i, j, Vec3f(1.0 - dx / centerX, 0.5 * (1.0 - light),
                                    1.0 - dy / centerY));
    }
  }
  ...
  for (auto i = 0; i < fb7.width(); ++i) {
    for (auto j = 0; j < fb7.height(); ++j) {
      ...
      // fb7.setPixelColor(i, j, sivelab::Vector3D(dCenter < 0.25 ? 1 : 0,
      // dCenter < 0.25 ? 1 : 0, dCenter < 0.25 ? 1 : 0));
      fb7.setPixelColor(
        i, j,
        Vec3f(dCenter < 0.25 ? dCenter : 1.0 - dCenter,
              dCenter < 0.25 ? dCenter : 1.0 - dCenter,
              dCenter < 0.25 ? 0.9 - 3.5 * dCenter * dCenter : 1.0 - dCenter));
    }
  }
